...
class Solder(Person):
    ...
    def folowToHero(self):
        self.file1 = open('hero_id.txt', 'r')
        self.line = self.file1.readline()
        if self.line != ",":
            print("Герой команды {} с ункальным номер {} взял в поход 1 солдата c уникальными id ({}) из команды {}.".format(self.team_id, self.line, self.id, self.team_id))
            self.file1.close()
teams = []
...
solder = int(input("Количество воинов: ")) + 1
for i in range(1, solder):
